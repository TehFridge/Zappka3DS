...
function draw_top_screen(dt)
    ...
    elseif state == "promki" then
		...
		if category ~= true then
			kurwalimit = #currentpromkitable
			if #currentpromkitable < 6 then
				for i = 1, #currentpromkitable do 
					TextDraw.DrawText(currentpromkitable[i + pagegap].coupon.targetPromotion.name, 27, 50 + 20 * i, {0.27,0.84,0.43, 1}, font, 1.9, true)
				end
			else 
				for i = 1, 6 do
					TextDraw.DrawText(currentpromkitable[i + pagegap].coupon.targetPromotion.name, 27, 50 + 20 * i, {0.27,0.84,0.43, 1}, font, 1.9, true)
		...
		elseif state == "promki" or state == "SSF" or state == "main_strona" then
			if intranet == "false" then
				...
				elseif state == "promki" then
					...
					textname = currentpromkitable[selectioncode + pagegap].coupon.targetPromotion.name
...
function updatepromki()
	...
	if code ~= 404 then
		category = false
		currentpromkitable = responded.data.ployOfferListing.ployOffers
	else
		category = true 
...
function love.update(dt)
    ...
    if collectgarbage("count") > memoryLimit then
        print("Memory limit exceeded, collecting garbage...")
